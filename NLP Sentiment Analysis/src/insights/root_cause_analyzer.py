...
Phrase frequency and sentiment association describe observed themes; they do
...
class RootCauseAnalyzer:
    """Analyze recurring themes in customer reviews without causal claims."""
    ...
    def analyze_negative_reviews(self, negative_texts: List[str], top_n: int = 20) -> Dict:
        ...
        stop_terms = {"review", "product", "use", "work", "time", "day", "one", "like", "get", "go", "see", "know", "take"}
        top_unigrams = [x for x in unigrams.most_common(top_n*2) if x[0] not in stop_terms and len(x[0]) > 2][:top_n]
        return {"top_unigrams": top_unigrams, "top_bigrams": bigrams.most_common(top_n), "top_trigrams": trigrams.most_common(top_n)}
    ...
    def get_root_cause_summary(self, negative_texts: List[str], all_negative_count: int) -> Dict:
        analysis = self.analyze_negative_reviews(negative_texts, top_n=15)
        return {
            "total_negative_reviews": int(all_negative_count),
            "top_negative_keywords": analysis["top_unigrams"],
            "top_negative_phrases": analysis["top_bigrams"],
            "interpretation_note": "Recurring terms/phrases are descriptive signals, not proven causes.",
        }
